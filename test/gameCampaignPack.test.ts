...
describe("game campaign foundations", () => {
  ...
  it("restores automatic turn-taking after Keeper send failure", async () => {
    const config = testConfig({ hermesEnabled: true });
    const preferences = GroupGamingPreferenceStore.fromConfig(config);
    preferences.set("group-a", {
      enabled: true,
  ...
  it("restores automatic turn-taking after AI response trigger failure", async () => {
    const config = testConfig({ hermesEnabled: true });
    const preferences = GroupGamingPreferenceStore.fromConfig(config);
    preferences.set("group-a", {
      enabled: true,
    ...
    const runtime = testGameRuntime(config, preferences, store, hermesDecisionResponse(), {
      sendGroupMessage: async (input) => ({
        ...
        idempotencyKey: input.idempotencyKey,
        aiResponseOk: false,
        aiResponseStatus: 500,
        aiResponseText: "ai trigger failed"
      }),
      updateGroupCurrentScene: async () => ({
        status: 200,
        ok: true
  ...
  it("restores automatic turn-taking when Hermes produces no Keeper message", async () => {
    const config = testConfig({ hermesEnabled: true });
    const preferences = GroupGamingPreferenceStore.fromConfig(config);
    preferences.set("group-a", {
      enabled: true,
  ...
  it("restores automatic turn-taking when an autonomous Keeper message is suppressed", async () => {
    vi.useFakeTimers();
    ...
    await runtime.handleGroupChatChanged(
      group({ useManualTurntaking: false }),
      groupNotification("doc-ai-1", "Velma studies the static.", "ai", "2026-06-06T00:00:10.000Z")
    );
    const result = await runtime.handleGroupChatChanged(
      group({ useManualTurntaking: false }),
      groupNotification("doc-user-1", "I check again immediately.", "user", "2026-06-06T00:00:20.000Z")
    );
  ...
  it("keeps a failed restore eligible for a later restore attempt", async () => {
    const config = testConfig({ hermesEnabled: true });
    const preferences = GroupGamingPreferenceStore.fromConfig(config);
    preferences.set("group-a", {
      enabled: true,
    ...
    const runtime = testGameRuntime(config, preferences, store, hermesDecisionResponse(), {
      ...
      updateGroupTurnTaking: async (input) => {
        turnTakingUpdates.push(input);
        if (!input.useManualTurntaking) {
          restoreAttempts += 1;
          if (restoreAttempts === 1) {
            return {
              status: 503,
              ok: false,
              responseText: "restore failed"
            };
          }
          return {
            status: 200,
    ...
    await runtime.handleGroupChatChanged(
      group({ useManualTurntaking: false }),
      groupNotification("doc-user-2", "I keep asking.", "user", "2026-06-06T00:02:00.000Z")
    );
