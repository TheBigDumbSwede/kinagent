...
The visible message is diegetic scene texture generated for the current situation:
...
In the current Hermes registry, `send_ambient_context_turn` is the active API for this in direct Kin chats. It is the
only current way for Hermes to inject immediate context without making the operational content overt in the visible
transcript.
...
context, `internet_response`, or the operational facts carried in the hidden packet.
...
context to a Kin without showing the operational details.
